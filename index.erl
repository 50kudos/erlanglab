...
lines_to_entry(L) -> lines_to_entry(L, [], 0).
lines_to_entry([],Es,_) -> Es;
...
% Look up a word in index entries,
% create or update the index for a word.
update_entry(W,[],Lth) ->
    [{W, [{Lth, Lth}]}];
...
% Receive list of index pairs {start,end}
% where it is a range of lines the word has been found.
update_index([{S,E}|[]],Lth) when E+1 < Lth -> [{S,E}, {Lth,Lth}];
update_index([{S,_E}|[]],Lth) -> [{S,Lth}];
update_index([I|Is],Lth) -> [I | update_index(Is,Lth)].
